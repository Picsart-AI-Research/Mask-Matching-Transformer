...
def build_optimizer(cfg, model):
    ...
    for module_name, module in model.named_modules():
        for module_param_name, value in module.named_parameters(recurse=False):
            ...
            # SVD finetune--set 'lr' of singular values 
            if "layer2" in module_name or "layer3" in module_name or "layer4" in module_name :
...
def main(args):
    ...
    # SVD finetune-- fix layer, convolutions
    for k, param in model.layer0.named_parameters():
        param.requires_grad = False
    for k, param in model.layer1.named_parameters():
        param.requires_grad = False
    for k, param in model.layer2.named_parameters():
        if 'vector_S' in k:
            param.requires_grad = True 
        else:
            param.requires_grad = False
    for k, param in model.layer3.named_parameters():
        if 'vector_S' in k:
            param.requires_grad = True 
        else:
            param.requires_grad = False        
    for k, param in model.layer4.named_parameters():
        if 'vector_S' in k:
            param.requires_grad = True 
        else:
            param.requires_grad = False
