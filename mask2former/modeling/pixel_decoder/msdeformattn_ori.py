...
class CrossAT4X(nn.Module):
    ...
    def forward(self, out_que, out_sup, pos_que, pos_sup, mask, mask_que):
        ...
        mask = F.interpolate(mask.unsqueeze(1).float(), size=(sh, sw), mode='bilinear')
        # mask_que = F.interpolate(mask_que.unsqueeze(1).float(), size=(h, w), mode='bilinear')

        ori_out_que = F.interpolate(out_que, size=(oh, ow), mode='bilinear').flatten(2).permute(2, 0, 1)
        ori_out_sup = F.interpolate(out_sup * mask, size=(osh, osw), mode='bilinear').flatten(2).permute(2, 0, 1)
        ori_pos_que = F.interpolate(pos_que, size=(oh, ow), mode='bilinear').flatten(2).permute(2, 0, 1)
        ori_pos_sup = F.interpolate(pos_sup * mask, size=(osh, osw), mode='bilinear').flatten(2).permute(2, 0, 1)
...
class MSDeformAttnPixelDecoder_ori(nn.Module):
    @configurable
    def __init__(
        self,
    ...
    def deformable_attention(self, features):
        srcs = []
        pos = []
        # Reverse feature maps into top-down order (from low to high resolution)
    ...
    def forward_process(self, features, out):
